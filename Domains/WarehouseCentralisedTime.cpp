#include "WarehouseCentralisedTime.h"

WarehouseCentralisedTime::~WarehouseCentralisedTime(void){
  delete whGraph ;
  whGraph = 0 ;
...
void WarehouseCentralisedTime::InitialiseMATeam(){
...
//  size_t nHid = 16 ; // fixed to compare against link agent formulation
  size_t nHid = nIn*4 ; // control for relative representational capacity
  Agent * neAgent ;
...
void WarehouseCentralisedTime::QueryMATeam(vector<size_t> memberIDs, vector<double> &a, vector<size_t> &s){
  vector<Edge *> e = whGraph->GetEdges() ;
...
void WarehouseCentralisedTime::GetJointState(vector<Edge *> e, vector<size_t> &s, vector<double> &eTime){
